...
class PluginFormcreator {
    constructor() {
        ...
        // Initialize jQuery extensions and event listeners
    ...
    initGridStack(sectionId) {
        ...
        // Check if the grid element exists before trying to initialize
        var gridEl = document.querySelector(selector);
        if (!gridEl) {
            console.warn('GridStack: Element not found for section ' + sectionId + ', retrying...');
    ...
    /**
     * Save a single item's position to the server
     */
    saveItemPosition(event, item) {
    ...
    cancelChangeItems(event, items) {
        const that = this;
        const grid = event.target.gridstack;
        $.each(items, function (index, item) {
            const id = $(item.el).attr('gs-id');
            ...
            grid.update(item.el, {
                x: that.initialPosition[id]['x'],
                y: that.initialPosition[id]['y'],
                w: that.initialPosition[id]['w'],
                h: that.initialPosition[id]['h']
            });
        });
    }
    ...
    deleteQuestion(target) {
        const item = $(target).closest('.grid-stack-item')[0];
        ...
        if (confirm(i18n.textdomain('formcreator').__('Are you sure you want to delete this question?', 'formcreator'))) {
            jQuery.ajax({
                ...
                const grid = item.parentElement.gridstack;
                if (grid) {
    ...
    toggleRequired(target) {
        const item = $(target).closest('.grid-stack-item');
        const id = item.attr('gs-id');
        if (typeof (id) === 'undefined') {
            return;
        }
        const required = $(target).hasClass('fa-check-circle');
        jQuery.ajax({
    ...
    moveQuestion(target, action) {
        ...
        const item = $(target).closest('.formcreator-question');
        const id = item.attr('gs-id');
        if (typeof (id) === 'undefined') {
            return;
        }
        ...
        switch (action) {
            case 'up':
                otherItem = item.prev('.formcreator-question');
                break;
            case 'down':
                otherItem = item.next('.formcreator-question');
                break;
        }
        if (otherItem.length < 1) {
            return;
        }
        const otherId = otherItem.attr('gs-id');
        const data = { move: {} };
        data.move[otherId] = {
    ...
    addQuestion() {
        ...
        $.ajax({
            ...
            const sectionId = form.find('select[name="plugin_formcreator_sections_id"]').val();
            const gridEl = $('[data-itemtype="PluginFormcreatorSection"][data-id="' + sectionId + '"] .grid-stack')[0];
    ...
    editQuestion() {
        ...
        $.ajax({
            ...
            const question = $('.plugin_formcreator_form_design[data-itemtype="PluginFormcreatorForm"] [data-itemtype="PluginFormcreatorQuestion"][gs-id="' + questionId + '"]');
    ...
    deleteSection(item) {
        if (confirm(i18n.textdomain('formcreator').__('Are you sure you want to delete this section?', 'formcreator'))) {
            const section = $(item).closest('#plugin_formcreator_form.plugin_formcreator_form_design [data-itemtype="PluginFormcreatorSection"]');
    ...
    moveSection(item, action) {
        ...
        $.ajax({
            ...
            success: function () {
                let otherSection;
                if (action == 'up') {
                    otherSection = section.prev('#plugin_formcreator_form.plugin_formcreator_form_design [data-itemtype="PluginFormcreatorSection"]').detach();
                    section.after(otherSection);
                }
                if (action == 'down') {
                    otherSection = section.next('#plugin_formcreator_form.plugin_formcreator_form_design [data-itemtype="PluginFormcreatorSection"]').detach();
    ...
    showQuestionForm(sectionId, questionId = 0) {
        const that = this;
        $.post({
    ...
    duplicateSection(item) {
        const section = $(item).closest('#plugin_formcreator_form.plugin_formcreator_form_design [data-itemtype="PluginFormcreatorSection"]');
        let sectionId = section.attr('data-id');
        const that = this;
        $.ajax({
            ...
            const lastSection = $('.plugin_formcreator_form_design[data-itemtype="PluginFormcreatorForm"] [data-itemtype="PluginFormcreatorSection"]').last();
    ...
    showSectionForm(formId, sectionId = 0) {
        const that = this;
        $.post({
    ...
    addSection() {
        const form = $('form[data-itemtype="PluginFormcreatorSection"]');
        const that = this;
        $.ajax({
            ...
            const addSectionRow = $('[data-itemtype="PluginFormcreatorForm"] li').last();
    ...
    editSection() {
        const form = $('form[data-itemtype="PluginFormcreatorSection"]');
        const sectionId = form.find('[name="id"]').val();
        const that = this;
        $.ajax({
            ...
            const section = $('.plugin_formcreator_form_design[data-itemtype="PluginFormcreatorForm"] [data-itemtype="PluginFormcreatorSection"][data-id="' + sectionId + '"]');
    ...
    createLanguage(formId, id = -1) {
        const placeholder = $('#plugin_formcreator_formLanguage');
    ...
    showTranslationEditor(object) {
        const formlanguageId = $(object).closest('[data-itemtype="PluginFormcreatorForm_Language"][data-id]').attr('data-id');
        const plugin_formcreator_translations_id = $(object).find('input[name="id"]').val();
    ...
    newTranslation(formLanguageId) {
        const modal = $(this.spinner)
    ...
    saveNewTranslation(element) {
        const that = this;
        const form = document.querySelector('form[name="plugin_formcreator_translation"]');
        tinyMCE.triggerSave();
        $.post({
    ...
    showUpdateTranslationForm(element) {
        const formLanguageId = $(element).closest('[data-itemtype="PluginFormcreatorForm_Language"][data-id]').attr('data-id');
        const translationId = $(element.closest('[data-itemtype="PluginFormcreatorTranslation"]')).attr('data-id');
        const modal = $(this.spinner);
...
const plugin_formcreator = new PluginFormcreator();
